...
app.get('/', noCacheMiddleware, async (req, res) => {
  try {
    ...
    const command = new CreateAnonymousWebExperienceUrlCommand({
      ...
      sessionDurationInMinutes: parseInt(process.env.SESSION_DURATION_MINUTES || '90', 10)
    });
    const response = await client.send(command);
    const anonymousUrl = response.anonymousUrl;
    
    // Session duration
    const sessionDuration = parseInt(process.env.SESSION_DURATION_MINUTES || '90', 10);
    ...
    res.send(`
      ...
      <body>
          ...
          <script>
            ...
            const sessionDuration = ${JSON.stringify(parseInt(sessionDuration, 90))}; // nosemgrep: unknown-value-with-script-tag
